...
export class DocumentRepository {
	...
	async patch(userId: string, documentId: string, patch: any): Promise<ResultType<{ updated: boolean }, { type: 'Forbidden'|'NotFound'|'Unknown'; message: string }>> {
		...
		let result
		try {
			result = await db.update(table.documents)
	...
	async getAccessList(userId: string, documentId: string): Promise<ResultType<any, { type: 'Forbidden'|'NotFound' }>> {
	...
	async patchAccess(userId: string, documentId: string, targetUserId: string, role: 'viewer' | 'editor' | 'owner'): Promise<ResultType<{ success: boolean }, { type: 'Forbidden'|'Unknown'; message: string }>> {
	...
	async revokeAccess(userId: string, documentId: string, targetUserId: string): Promise<ResultType<{ success: boolean }, { type: 'Forbidden'|'Unknown'; message: string }>> {
		const isOwner = await db
			.select()
			.from(table.documentAccess)
			.where(and(
	...
	async createLink(userId: string, documentId: string, origin: string): Promise<ResultType<{ linkId: string, url: string, expiresAt: string }, { type: 'Forbidden'|'NotFound'|'Unknown'; message: string }>> {
		const access = await db
			.select()
			.from(table.documentAccess)
			.where(
	...
	async downloadByLink(filename: string): Promise<ResultType<{ content: string, title: string, fileType: string, fileMimeType: string, fileExtension: string }, { type: 'NotFound'|'Gone' }>> {
